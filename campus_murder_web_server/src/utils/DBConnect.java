...
public class DBConnect {
	...
	private DBConnect(){
		try {
			...
			retriveUserSessionData = con.prepareStatement("select i.session_name session_name, i.points points, "
					...
					+ "where i.username=?;");
			insertKillRequest = con.prepareStatement("insert into waiting_confirmation_kills set killer=?,killed=?,session=?,place=?,object=?,time=now()");
			getPendingKill = con.prepareStatement("select w.killer killer, u.profile_image killer_image, w.place place, p.image place_image, w.object object, o.image object_image "
			...
			confirmBeingKilled = con.prepareStatement("insert into kills " + 
					"select * from waiting_confirmation_kills " + 
			...
			startSession = con.prepareStatement("update inscriptions old set " + 
					...
					"where old.session_name=? and old.actual_user_target is null;");
	...
	public void killRequest(String killer, String killed, String session, String place, String object) {
		try {
